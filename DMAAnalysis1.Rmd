...
```{r}
fastMA <- as.numeric(unique(stats$ma_fast))
slowMA <- as.numeric(unique(stats$ma_slow))
for (sma in slowMA){
  p <- ggplot()
  for(fma in fastMA){
    dfName <- as.character(paste(fma,sma,"stats",sep = ""))
    subsdf <- subset(portfolio_avg, ma_fast == fma & ma_slow == sma)
    assign(dfName,subsdf)
    p <- p + geom_point(data=subsdf,aes(x=atr,y=Num.Trades))
    }
p
plotName <- as.character(paste("av_stats",sma,sep=""))
assign(plotName,p)
}
av_stats100
```
